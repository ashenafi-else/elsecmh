...
class TestImportAxFMetaData(TestBaseCmh):
    ...
    def test_import_axf_meta_data(self):
        ...
        with self.put_asset_to_buff_patch:
            ...
            self.assertEqual(str(axf_asset.dam_uuid), self.material_asset_dto.dam_uuid)
            self.assertEqual(axf_asset.filename, self.material_asset_dto.filename)
            # TODO (f.gaponenko@invento.by): fix test
            # self.assertEqual(axf_asset.size, self.material_asset_dto.size)
            self.assertEqual(axf_asset.extension, self.material_asset_dto.extension)
            self.assertEqual(axf_asset.brand_id, self.material_asset_dto.brand_id)
    ...
    def test_import_with_wrong_extension(self):
        ...
        with self.put_asset_to_buff_patch:
            with self.assertRaises(RuntimeError):
                self.import_axf_meta_data_dto.dam_asset_info.extension = AssetExtensions.BLEND
    ...
    def test_import_with_wrong_resource_type(self):
        ...
        with self.put_asset_to_buff_patch:
            with self.assertRaises(RuntimeError):
                self.import_axf_meta_data_dto.dam_asset_info.resource_type = ResourceTypes.BLENDER_MODEL
                self.import_op(self.import_axf_meta_data_dto)
