...
class TestImportAxFBufferAssetHandler(TestBaseCmh):
    ...
    def setUp(self):
        ...
        self.material_revision = MaterialRevision.objects.create(
            material=test_material, material_info={})
        test_material.active_revision = self.material_revision
        test_material.save()

        Router[ImportAxFBufferAssetSuccessHandlerInterface] = OperationType(
            expose=False, producer=TransportABC)
        self.import_axf_handler_op = \
            Router[ImportAxFBufferAssetSuccessHandlerInterface.uri]
    ...
    def test_import_axf_buffer_asset_handler(self):
        ...
        validate_result = self.import_axf_handler_op(
            self.put_asset_to_buff_result, **context)
        self.assertIsNone(validate_result)
        material_revision = MaterialRevision.objects.filter(
            uuid=self.material_revision.uuid).first()
        raw_data = material_revision.meta_data.filter(
            category=MaterialMetaData.AXF_RAW_DATA).first()
        self.assertEqual(raw_data.key, 'data')
        meta_data = material_revision.meta_data.filter(
            category=MaterialMetaData.AXF_META_DATA).first()
